...
const defaultPostNavigationFilterForGroup = (group) => {
  if (
    ...
    allowedPostNavigationFilters.indexOf(
      group.configuration.forcePostSortMethodAs
    ) > -1
  ) {
    ...
    if (group.configuration.canAddNewPosts === true) {
      ...
      group.configuration.canAddNewPosts === false &&
...
const getRandomPostNavigationPage = async (
  statusFilter,
  where,
  pageOffset,
  randomSeed
...
const getRandomPostNavigationInfo = async (
  currentPost,
  statusFilter,
  categoryId,
  randomSeed
  ...
  const currentPageOffset =
    Math.floor(countBeforeCurrent / postNavigationPageSize) *
    postNavigationPageSize;
  const currentPageRows = await getRandomPostNavigationPage(
    statusFilter,
    where,
    currentPageOffset,
    randomSeed
  );
  const currentPageIndex = currentPageRows.findIndex(
  ...
  let previousPostId =
    currentPageIndex > 0
      ...
      : undefined;
  let nextPostId =
    currentPageIndex < currentPageRows.length - 1
      ? currentPageRows[currentPageIndex + 1].id
      : undefined;

  if (!previousPostId && currentPageOffset > 0) {
    const previousPageRows = await getRandomPostNavigationPage(
      statusFilter,
      where,
      currentPageOffset - postNavigationPageSize,
      randomSeed
    );
  ...
  if (
    ...
    const nextPageRows = await getRandomPostNavigationPage(
      statusFilter,
      where,
      currentPageOffset + postNavigationPageSize,
      randomSeed
...
const getWindowPostNavigationInfo = async (
  group,
  currentPost,
  statusFilter,
  filter,
  ...
  return {
    previousPostId:
      rows.find((row) => Number(row.row_number) === Number(currentRow.row_number) - 1)
        ?.id,
    nextPostId:
      rows.find((row) => Number(row.row_number) === Number(currentRow.row_number) + 1)
        ?.id,
...
const getPostNavigationInfo = async (
  group,
  currentPost,
  statusFilter,
  filter,
  ...
  if (filter === "random") {
    return getRandomPostNavigationInfo(
      currentPost,
      statusFilter,
      categoryId,
      randomSeed
    ...
    return getWindowPostNavigationInfo(
      group,
      currentPost,
      statusFilter,
      filter,
...
router.get("/:id/navigation", auth.can("view post"), async function (req, res) {
  ...
  try {
    ...
    const defaultStatusFilter = officialStatusToPostNavigationStatus(
      currentPost.official_status
    );
    ...
    const navigationInfo = await getPostNavigationInfo(
      group,
      currentPost,
      statusFilter,
      filter,
    ...
    if (!navigationInfo) {
      sendPostOrError(
        ...
        "navigation",
        req.user,
        "Post not found in navigation list",
        404
      );
      return;
    }
