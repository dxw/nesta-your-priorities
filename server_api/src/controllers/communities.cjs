...
const getUsersForCommunity =
  require("../utils/export_utils.cjs").getUsersForCommunity;
...
const ensureFraudDetectionEnabledForCommunity = async function (
  req,
  res,
  communityId,
  context
        ...
        if (req.user) {
          ...
          async.parallel(
            ...
            function (error) {
              ...
              combinedCommunities = _.concat(
                adminCommunities,
                combinedCommunities
              );
              combinedCommunities = _.uniqBy(
                combinedCommunities,
                function (community) {
                  return community.id;
                }
              ...
              if (error) {
                ...
                combinedGroups.forEach((group) => {
                  if (group.GroupLogoImages) {
                    group.GroupLogoImages = _.orderBy(
                      group.GroupLogoImages,
                      ["created_at"],
                      ["asc"]
                    );
                  }
                  if (group.GroupHeaderImages) {
                    group.GroupHeaderImages = _.orderBy(
                  ...
                  if (group.Category && group.Category.CategoryIconImages) {
                    group.Category.CategoryIconImages = _.orderBy(
                      group.Category.CategoryIconImages,
                      ["updated_at"],
                      ["asc"]
                    );
                  }
                  if (group.Categories) {
                    group.Categories = _.orderBy(
                      group.Categories,
                      ["name"],
                      ["asc"]
                    );
...
const updateCommunityConfigParameters = function (req, community) {
  ...
  community.set(
    ...
    req.body.disableCollectionUpLink && req.body.disableCollectionUpLink != ""
      ? true
      : false
  );
  community.set(
    "configuration.disableDomainUpLink",
    req.body.disableDomainUpLink && req.body.disableDomainUpLink != ""
      ? true
      : false
  );
  community.set(
  ...
  community.set(
    "configuration.facebookPixelId",
    req.body.facebookPixelId && req.body.facebookPixelId != ""
  ...
  community.set(
    "configuration.redirectToGroupId",
    req.body.redirectToGroupId && req.body.redirectToGroupId != ""
  ...
  community.set(
    "configuration.welcomeHTML",
    req.body.welcomeHTML && req.body.welcomeHTML != ""
  ...
  community.set(
    ...
    req.body.communityId && req.body.communityId != ""
      ? req.body.communityId
      : null
  );

  community.set(
    "configuration.customSamlDeniedMessage",
    req.body.customSamlDeniedMessage && req.body.customSamlDeniedMessage != ""
      ? req.body.customSamlDeniedMessage
      : null
  );
  community.set(
    "configuration.customSamlLoginMessage",
    req.body.customSamlLoginMessage && req.body.customSamlLoginMessage != ""
  ...
  community.set(
    "configuration.useCommunityIdForAnalytics",
    truthValueFromBody(req.body.useCommunityIdForAnalytics)
  );

  community.set(
    "configuration.disableGroupDynamicFontSizes",
    truthValueFromBody(req.body.disableGroupDynamicFontSizes)
  );
  community.set(
    "configuration.hideGroupListCardObjectives",
    truthValueFromBody(req.body.hideGroupListCardObjectives)
  );
  ...
  if (truthValueFromBody(req.body.appHomeScreenIconImageId)) {
    community.set(
      "configuration.appHomeScreenIconImageId",
      req.body.appHomeScreenIconImageId
    );
  }
  ...
  community.set(
    "configuration.appHomeScreenShortName",
    req.body.appHomeScreenShortName && req.body.appHomeScreenShortName != null
      ? req.body.appHomeScreenShortName
      : null
  );
  community.set(
    "configuration.signupTermsPageId",
    req.body.signupTermsPageId && req.body.signupTermsPageId != ""
  ...
  community.set(
    ...
    truthValueFromBody(req.body.alwaysShowOnDomainPage)
  );
  community.set(
    "configuration.useReadMoreForDescription",
    truthValueFromBody(req.body.useReadMoreForDescription)
  );
  ...
  community.set(
    "configuration.muteNotificationsForEndorsements",
    truthValueFromBody(req.body.muteNotificationsForEndorsements)
  );
  community.set(
  ...
  community.set(
    "configuration.enableFraudDetection",
    truthValueFromBody(req.body.enableFraudDetection)
  );
  community.set(
    "configuration.useZiggeo",
    truthValueFromBody(req.body.useZiggeo)
  );
  community.set(
    "configuration.highlightedLanguages",
    req.body.highlightedLanguages && req.body.highlightedLanguages != ""
      ? req.body.highlightedLanguages
      : null
  );
  community.set(
    "configuration.actAsLinkToExternalYPSite",
...
router.get(
  "/:communityFolderId/communityFolders",
  auth.can("view community"),
  function (req, res) {
    getCommunityFolder(
      req,
      req.params.communityFolderId,
      function (error, communityFolder) {
        if (error) {
...
router.delete(
  "/:communityId/:activityId/delete_activity",
...
router.delete(
  "/:communityId/remove_many_admins",
  auth.can("edit community"),
  (req, res) => {
    queue.add(
      "process-deletion",
...
router.delete(
  "/:communityId/remove_many_promoters",
  auth.can("edit community"),
  (req, res) => {
    queue.add(
      "process-deletion",
...
router.delete(
  "/:communityId/remove_many_users_and_delete_content",
  auth.can("edit community"),
  function (req, res) {
    queue.add(
      "process-deletion",
...
router.delete(
  "/:communityId/:userId/remove_and_delete_user_content",
  auth.can("edit community"),
  function (req, res) {
    getCommunityAndUser(
      req.params.communityId,
...
router.delete(
  "/:communityId/:userId/remove_admin",
  auth.can("edit community"),
  function (req, res) {
    getCommunityAndUser(
      req.params.communityId,
...
router.delete(
  "/:communityId/:userId/remove_promoter",
  auth.can("edit community"),
  function (req, res) {
    getCommunityAndUser(
      req.params.communityId,
...
router.delete(
  "/:communityId/:userId/remove_user",
  auth.can("edit community"),
  function (req, res) {
    getCommunityAndUser(
      req.params.communityId,
...
router.post(
  "/:communityId/:email/add_promoter",
  auth.can("edit community"),
  function (req, res) {
    getCommunityAndUser(
      req.params.communityId,
...
router.post(
  "/:communityId/add_page",
  auth.can("edit community"),
  function (req, res) {
    models.Page.newPage(
      req,
      { community_id: req.params.communityId, content: {}, title: {} },
      function (error, pages) {
        if (error) {
...
router.put(
  "/:communityId/:pageId/update_page_locale",
  auth.can("edit community"),
  function (req, res) {
    models.Page.updatePageLocale(
      req,
      { community_id: req.params.communityId, id: req.params.pageId },
      function (error) {
        if (error) {
...
router.put(
  "/:communityId/:pageId/update_page_weight",
  auth.can("edit community"),
  function (req, res) {
...
router.put(
  "/:communityId/:pageId/publish_page",
  auth.can("edit community"),
  function (req, res) {
...
router.put(
  "/:communityId/:pageId/un_publish_page",
  auth.can("edit community"),
  function (req, res) {
...
router.delete(
  "/:communityId/:pageId/delete_page",
  auth.can("edit community"),
  function (req, res) {
  ...
  function (req, res) {
    if (req.query.textType && req.query.textType.indexOf("community") > -1) {
      ...
      sendCommunityOrError(
        ...
        "translated",
        req.user,
        "Wrong textType or missing textType",
        401
      );
    }
  }
  ...
  community
    ...
    .then(function () {
      ...
      queue.add(
        ...
        { type: "update-collection", communityId: community.id },
        "low"
      );
      community.updateAllExternalCounters(
        req,
        "up",
        "counter_communities",
        function () {
...
router.delete(
  "/:communityId/:itemId/:itemType/:actionType/process_one_moderation_item",
  auth.can("edit community"),
  (req, res) => {
...
router.delete(
  "/:communityId/:actionType/process_many_moderation_item",
  auth.can("edit community"),
  (req, res) => {
    queue.add(
      "process-moderation",
...
router.get(
  "/:communityId/moderate_all_content",
  auth.can("edit community"),
  (req, res) => {
    getAllModeratedItemsByCommunity(
...
router.get(
  "/:communityId/flagged_content_count",
  auth.can("edit community"),
  (req, res) => {
    countAllModeratedItemsByCommunity(
      { communityId: req.params.communityId },
        ...
        models.Community.findOne({
          ...
          .then(function (model) {
            if (model) {
              ...
              var communityName = sanitizeFilename(model.name).replace(
                / /g,
                ""
              );
              var dateString = moment(new Date()).format("DD_MM_YY_HH_mm");
              var filename =
                "users_export_for_community_id_" +
                model.id +
                "_" +
                communityName +
                "_" +
          ...
          models.Community.findOne({
            ...
            .then(function (model) {
              if (model) {
                ...
                var dateString = moment(new Date()).format("DD_MM_YY_HH_mm");
                var filename =
                  "logins_export_for_community_id_" +
                  model.id +
                  "_" +
                  communityName +
                  "_" +
...
router.get(
  "/:communityId/:ssnListId/ssn_login_list_count",
  auth.can("edit community"),
  function (req, res) {
...
router.delete(
  "/:communityId/:ssnListId/delete_ssn_login_list",
  auth.can("edit community"),
  function (req, res) {
...
router.post(
  "/:communityId/upload_ssn_login_list",
  auth.can("edit community"),
  function (req, res) {
...
router.get("/:id/stats_posts", auth.can("edit community"), function (req, res) {
  countModelRowsByTimePeriod(
    req,
    "stats_posts_" + req.params.id + "_community",
    models.Post,
    {},
    getCommunityIncludes(req.params.id),
    (error, results) => {
...
router.get(
  ...
  function (req, res) {
    countModelRowsByTimePeriod(
      req,
      "stats_points_" + req.params.id + "_community",
      models.Point,
      {},
      getPointCommunityIncludes(req.params.id),
      (error, results) => {
        sendBackAnalyticsResultsOrError(req, res, error, results);
      }
    );
...
router.get("/:id/stats_votes", auth.can("edit community"), function (req, res) {
  countModelRowsByTimePeriod(
    req,
    "stats_votes_" + req.params.id + "_community",
    models.AcActivity,
    {
      type: {
        $in: [
...
router.put(
  "/:communityId/:type/start_report_creation",
  auth.can("edit community"),
  async function (req, res) {
    try {
      ...
      if (
        req.params.type === "fraudAuditReport" &&
        !(await ensureFraudDetectionEnabledForCommunity(
          req,
          res,
          communityId,
...
router.get(
  "/:communityId/:jobId/report_creation_progress",
  auth.can("edit community"),
  async function (req, res) {
    try {
      const communityId = parseInt(req.params.communityId);
      ...
      if (
        internalData.reportType === "start-fraud-audit-report-generation" &&
        !(await ensureFraudDetectionEnabledForCommunity(
          req,
          res,
          communityId,
          "report_creation_progress"
        ))
      ) {
        return;
      }
...
router.put(
  "/:communityId/:type/:selectedMethod/:collectionType/start_endorsement_fraud_action",
  auth.can("edit community"),
  async function (req, res) {
    try {
      const communityId = parseInt(req.params.communityId);
      ...
      if (
        !(await ensureFraudDetectionEnabledForCommunity(
          ...
          res,
          communityId,
          "start_endorsement_fraud_action"
        ))
      ) {
        return;
      }
    ...
    try {
      ...
      if (
        !(await ensureFraudDetectionEnabledForCommunity(
          ...
          res,
          communityId,
          "endorsement_fraud_action_status"
        ))
      ) {
        return;
      }
...
router.get(
  "/:communityId/:type/getPlausibleSeries",
  auth.can("edit community marketing"),
  async (req, res) => {
...
router.put(
  "/:communityId/plausibleStatsProxy",
  auth.can("edit community marketing"),
  async (req, res) => {
    ...
    try {
      const plausibleData = await plausibleStatsProxy(
        req.body.plausibleUrl,
        plausibleParams
      );
      res.send(plausibleData);
    } catch (error) {
...
router.put(
  "/:communityId/:campaignId/update_campaign",
  auth.can("edit community marketing"),
  async (req, res) => {
    try {
...
router.delete(
  "/:communityId/:campaignId/delete_campaign",
  auth.can("edit community marketing"),
  async (req, res) => {
    try {
      const campaign = await models.Campaign.findOne({
...
router.get(
  "/:communityId/group_folders_simple",
  auth.can("edit community"),
  async (req, res) => {
    try {
