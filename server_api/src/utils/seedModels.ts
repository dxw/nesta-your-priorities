...
if (env === "production") {
  ...
  if (
    ...
    log.error(
      "One or more YP_DEV_DATABASE environment variables are not set for development."
    );
    process.exit(1);
  }
  try {
...
const mainDb: { [key: string]: any } = {};

// Compound index commands from server_api/models/index.cjs
const mainCompoundIndexCommands: string[] = [
...
async function seedAllModels() {
  ...
  if (env !== "production") {
    if (
      ...
      log.error(
        "Missing YP_DEV_DATABASE environment variables for main database in non-production."
      );
      process.exit(1);
    }
  } else {
